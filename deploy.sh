...
deploy_local() {
    ...
    tmux send-keys -t "=${session}:=directory" "target/$mode/directory_service 0.0.0.0 $dirkey $dircrt -l $pathlen -k $numrounds -d $rounddur -w $roundwait" Enter
    sleep 1
    echo " Done"
    ...
    for i in `seq 1 $n`; do
        ...
        tmux send-keys -t "=${session}:=mix-$i" "target/$mode/mix $localhost:$port $dirdom --config $mixcfg -t $threads -p $dirport -c $cacrt $x25519" Enter
        echo " Done"
    done
...
deploy_testbed() {
    ...
    tmux new-session -d -s $session
    echo -n "Starting directory server on port $dirport ..."
    tmux send-keys -t "=${session}:" "target/$mode/directory_service 0.0.0.0 $dirkey $dircrt -l $pathlen -k $numrounds -d $rounddur -w $roundwait" Enter
    sleep 1
    echo " Done"
    ...
    for i in `seq -w 01 $n`; do
        ...
        # start the engine :)
        # TODO avoid SPACE?
        ssh $mix tmux send-keys -t "=${session}:" ".testbed/mix Space $addr:$port Space $dirdom Space --config Space $mixcfg Space -t Space $threads Space -p Space $dirport Space -c Space $cacrt Space $x25519" Enter
    done
}
